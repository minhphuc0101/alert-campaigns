...
def check_all_nested_formulas():
    ...
    try:
        ...
        return [f"Γ¥î Kh├┤ng thß╗â truy cß║¡p file ch├¡nh: {e}"]
    ...
    for row in all_records:
        ...
        # D├╣ng Job ID l├ám t├¬n hiß╗ân thß╗ï, nß║┐u kh├┤ng c├│ th├¼ fallback vß╗ü Campaign name
        ...
        for attempt in range(max_retries):
            try:
                ...
                for i, r in enumerate(values):
                    if 'Hß║íng mß╗Ñc' in r and 'Cost' in r:
                        header_row_idx = i
                        break
                ...
                hang_muc_idx = headers.index('Hß║íng mß╗Ñc')
                    ...
                    if hang_muc.lower() == 'total' or hang_muc == '':
                        ...
                        if hang_muc.lower() == 'total':
                            ...
                            if expected_sum not in cost_form.replace("$", ""):
                                alerts.append(f"ΓÜá∩╕Å **{campaign}**: H├ám SUM ß╗ƒ d├▓ng Total (Cost) bß╗ï sai dß║úi dß╗» liß╗çu! (C├┤ng thß╗⌐c hiß╗çn tß║íi: `{cost_form}`, ─É├íng lß║╜ phß║úi l├á: `{expected_sum}`)")
                        break
                        
                    hm_lower = hang_muc.lower()
                    if "fb" in hm_lower or "facebook" in hm_lower:
                        if "*1.05" not in cost_form.replace(" ", ""):
                            alerts.append(f"Γ¥î **{campaign}**: Hß║íng mß╗Ñc `{hang_muc}` thiß║┐u thuß║┐ 5% FB! (C├┤ng thß╗⌐c hiß╗çn tß║íi: `{cost_form}`)")
                    elif "adx" in hm_lower or "zalo" in hm_lower:
                        if "/1.08" not in cost_form.replace(" ", ""):
                            alerts.append(f"Γ¥î **{campaign}**: Hß║íng mß╗Ñc `{hang_muc}` thiß║┐u trß╗½ VAT 8% ADX/Zalo! (C├┤ng thß╗⌐c hiß╗çn tß║íi: `{cost_form}`)")
                    elif "gg" in hm_lower or "google" in hm_lower:
                        if "1.05" in cost_form or "1.08" in cost_form:
                            alerts.append(f"Γ¥î **{campaign}**: Hß║íng mß╗Ñc `{hang_muc}` (GG) kh├┤ng ─æ╞░ß╗úc t├¡nh thuß║┐, nh╞░ng c├┤ng thß╗⌐c c├│ vß║╗ sai! (C├┤ng thß╗⌐c hiß╗çn tß║íi: `{cost_form}`)")
                ...
                if "429" in str(e) or "Quota" in str(e):
                    if attempt < max_retries - 1:
                        ...
                        alerts.append(f"ΓÅ▒ **{campaign}**: Qu├í giß╗¢i hß║ín API cß╗ºa Google (Rate limit). ─É├ú thß╗¡ lß║íi 3 lß║ºn nh╞░ng thß║Ñt bß║íi!")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "429" in str(e) or "Quota" in str(e):
                    ...
                    alerts.append(f"≡ƒöÆ **{campaign}**: Bot kh├┤ng c├│ quyß╗ün truy cß║¡p file PNL! H├úy share file cho bot.")
